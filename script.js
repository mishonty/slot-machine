    ...
    spin() {
        ...
        for (let row = 0; row < this.rowsCount; row++) {
            for (let reel = 0; reel < this.reelsCount; reel++) {
                const reelLength = this.reels[reel].length;
                const i = Math.floor(Math.random() * reelLength);
                const s = this.reels[reel][i];
                //console.log(`Index: ${i}. Symbol: ${s}`);
                result.reelsIndex.push(i);
                result.symbols[row].push(s);
            }
        }

        for (let i = 0; i < this.lines.length; i++) {
            const line = this.lines[i];
            const lineSymbol = result.symbols[line[0]][0];
            let count = 1;
            let linePayout = 0;
            console.log(`Line: ${line}. First symbol: ${lineSymbol}`);

            for (let j = 1; j < this.reelsCount; j++) {
                const currentSymbol = result.symbols[line[j]][j];
            ...
            if (count >= 3) {
                linePayout = this.symbols[lineSymbol][count - 1];
                result.totalPayout += linePayout;
                result.linesPayout[i + 1] = linePayout;
                console.log(`You won! ${linePayout} on line #${i + 1}.`);
            }
        }
...
const printResult = game.spin();
console.log('--------------------------------------------');
console.log('Total win:', printResult.totalPayout);
console.log('Payout by lines:', printResult.linesPayout);
console.log('Symbols on display:', printResult.symbols);
